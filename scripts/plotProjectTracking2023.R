#' Simplified plot of green hydrogen announcements and flows for 2023
#' 
plotProjectTracking2023 <- function(data.track.calc,
                                    shift.incoming,
                                    shift.outgoing,
                                    y.upper,
                                    title,
  ...
  # Put delayed and disappeared bars from stat21t22 into year2022
  # and from stat22t23 into year2023, and from stat23t23out into year2023out
  data.track.calc <- data.track.calc %>% 
    ...
    # Put stat23t23out disappeared into status2023out
    mutate(status2023out = ifelse(isEq(stat23t23out, "No info"), "No info", status2023out),
           stat23t23out = ifelse(isEq(stat23t23out, "No info"), NA_character_, stat23t23out)) %>% 
    # Put stat23t23out delayed into status2023out
    mutate(status2023out = ifelse(isEq(stat23t23out, "Delayed/out"), "Delayed/out", status2023out),
           stat23t23out = ifelse(isEq(stat23t23out, "Delayed/out"), NA_character_, stat23t23out)) %>%
    ...
    mutate(stat21t22 = ifelse(isEq(stat21t22, c("Cap/out", "Cap/in")), NA_character_, stat21t22),
           ...
           stat23t23out = ifelse(isEq(stat23t23out, c("Cap/out", "Cap/in")), NA_character_, stat23t23out)) %>% 
    ...
    # Add flow of delayed and disappeared projects from status2023 to status2023out
    mutate(status2023out = ifelse(isEq(status2023, "Delayed/out"), "Delayed/out", status2023out),
           status2023out = ifelse(isEq(status2023, "No info"), "No info", status2023out)) %>% 
    ...
    mutate(status2022 = ifelse(isEq(status2022, "Operational") &
           ...
           status2023out = ifelse(isEq(status2023out, "Operational") &
                                    !is.na(stat23t23out), "Newly operational", status2023out),
           # For newly operational, add flow from status2022 to status2023
           status2023 = ifelse(isEq(status2022, "Newly operational"), "Newly operational", status2023),
           # For newly operational, add flow from status2023 to status2023out
           status2023out = ifelse(isEq(status2023, "Newly operational"), "Newly operational", status2023out)) %>% 
    ...
    mutate(stat21t22 = ifelse(isEq(stat21t22, "Delayed/in"), "New", stat21t22),
           ...
           stat23t23out = ifelse(isEq(stat23t23out, "Delayed/in"), "New", stat23t23out),
           stat23t23out = ifelse(isEq(stat23t23out, "Early"), "New", stat23t23out))
  
  # Select required columns
  df <- data.track.calc %>% 
    select(status2021, stat21t22, status2022, stat22t23, status2023, stat23t23out, status2023out, capacity) %>% 
    rowwise() %>% 
    # Remove projects without any stratum
    filter(any(!is.na(status2021), !is.na(stat21t22), !is.na(status2022),
               !is.na(stat22t23), !is.na(status2023), !is.na(stat23t23out),
               !is.na(status2023out)))
  ...
  # Order of axis items
  x.order <- c("status2021", "stat21t22", "status2022", "stat22t23", "status2023", "stat23t23out", "status2023out")
  ...
  cap.2023 <- data.track.calc %>% 
    ...
    sum()
  # Status 2023out
  cap.2023out <- data.track.calc %>% 
    filter(!is.na(status2023out), !(status2023out %in% c("Delayed/out", "No info"))) %>% 
    pull(capacity) %>% 
    sum()
  ...
  if (annotate.gap == TRUE) {
    ...
    p <- p +
      ...
      geom_segment(aes(x = 1, xend = 7.8, y = cap.2021, yend = cap.2021),
                   linetype = "dashed",
                   linewidth = 0.25) +
      # Dashed line from 2023out
      geom_segment(aes(x = 7, xend = 7.8, y = cap.2023out, yend = cap.2023out),
                   linetype = "dashed",
                   linewidth = 0.25) +
      # Arrow
      geom_segment(aes(x = 7.7, xend = 7.7, y = cap.2021, yend = cap.2023out),
                   arrow = arrow(length = unit(1.5, "mm"), ends = "both"),
                   linewidth = 0.5) +
      # Annotation
      geom_text(aes(x = 7.75, y = mean(c(cap.2021, cap.2023out))),
  ...
  if (annotate.labels == TRUE){
    ...
    data.annotations.status <- data.track.calc %>%
      ...
      add_row(x = "stat23t23out", y = 0, label = "", alpha = 0) %>% 
      add_row(x = "status2023out", y = 0, label = "", alpha = 0) %>% 
      mutate(x = factor(x, levels = c("status2021", "stat21t22", "status2022", "stat22t23", "status2023", "stat23t23out", "status2023out")))
    ...
    # Add annotations for outcome in status2023out
    data.annotations.outcome <- data.track.calc %>% 
      filter(!is.na(status2023out)) %>% 
      group_by(status2023out) %>% 
      summarise(sum = sum(capacity)) %>% 
      arrange(match(status2023out, node.order)) %>% 
      transmute(x = "status2023out",
                y = cumsum(sum) - 0.5*sum,
                label = status2023out,
                alpha = 1) %>%
    ...
    p <- p +
      ...
      geom_label_repel(data = data.annotations.total,
                       ...
                       size = 5/.pt,
                       nudge_x = c(-0.6, 0, -0.6, 0, -0.6, 0, -0.6),
                       nudge_y = c(shift.outgoing, 0, shift.outgoing, 0, shift.outgoing, 0, shift.outgoing),
                       label.padding = unit(0.15, "lines"),
                       segment.curvature = c(-0.3, 0, -0.3, 0, -0.3, 0, -0.3),
                       segment.ncp = 10,
                       segment.size = 0.25,
      ...
      geom_text(data = data.annotations.outcome.failed,
                mapping = aes(x = 7.15, y = y, label = label, alpha = alpha),
                size = 5 /.pt,
                hjust = 0,
                lineheight = 1) +
      geom_text_repel(data = data.annotations.outcome.success,
                      mapping = aes(x = 7.1, y = y, label = label, alpha = alpha),
                      size = 5 /.pt,
                      nudge_x = 0.2,
                      direction = "y",
                      segment.size = 0.25,
  ...
  if (annotate.gap == TRUE){
    p <- p +
      ...
      coord_cartesian(xlim = c(0.5, 7.1), clip = "off")
  }
